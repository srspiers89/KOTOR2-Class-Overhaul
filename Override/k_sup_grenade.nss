...
void main()
{
    ...
    if (IsObjectPartyMember(OBJECT_SELF))
        nDC = GetSkillRank(SKILL_DEMOLITIONS, OBJECT_SELF);
    else
        nDC = GetHitDice(GetPartyMemberByIndex(0)) + 5;

    if (IsObjectPartyMember(OBJECT_SELF))
        nDamage = d4(GetSkillRank(SKILL_DEMOLITIONS, OBJECT_SELF));
    else
        nDamage = d4(GetHitDice(GetPartyMemberByIndex(0)) + 5);
        ...
        while(GetIsObjectValid(oTarget))
        {
            if(!GetIsNeutral(oTarget))
            {
                ...
                if(!ReflexSave(oTarget, nDCCheck, SAVING_THROW_TYPE_NONE))
                {
                    ...
                    {
                        effect eBleed = EffectDamage(2, DAMAGE_TYPE_ACID);
                        SP_InterativeDamage(eBleed, 31, oTarget);
                    }
                }
                else
                {
        ...
        while(GetIsObjectValid(oTarget))
        {
            if(!GetIsNeutral(oTarget))
            {
                ...
                if(!WillSave(oTarget, nDCCheck, SAVING_THROW_TYPE_SONIC))
                {
                    if(GetRacialType(oTarget) == RACIAL_TYPE_DROID)
                    {
                        ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eDroidStun, oTarget, 6.0);
                    }
                    else
                    {
    ...
    else if((nSpell == 89) || (nSpell == 248)) // Thermal Detonator (Normal and Launched)
    {
        ...
        oTarget = GetFirstObjectInShape(SHAPE_SPHERE, 12.0, GetSpellTargetLocation(), FALSE);
        while(GetIsObjectValid(oTarget))
        {
            if(!GetIsNeutral(oTarget))
            {
    ...
    else if((nSpell == 94) || (nSpell == 253)) // Plasma Grenade (Normal and Launched)
    {
        ...
        {
            if (GetEffectType(eEffect) == EFFECT_TYPE_ENTANGLE)
                nDamage += 30;
