...
void main()
{
    ...
    if(!CUSTOM_DupEffect(nID))
    {
        ...
        // DJS-OEI 1/27/2004
        // Duration is now partially based on the Treat Injury skill
        // of the user of the stimulant.
        float fDuration = 120.0;
        int nSkill = GetSkillRank( SKILL_TREAT_INJURY, OBJECT_SELF );
        if( nSkill > 0 ) {
            fDuration += 2.0 * nSkill;
        }

        ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, GetSpellTargetObject(), fDuration);

        if(nID == 75 || nID == 76 || nID == 77)
        {
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eBonusHP, GetSpellTargetObject(), fDuration);
        }
    }
}
